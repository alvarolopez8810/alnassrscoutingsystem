...
def load_squad_data(category):
    ...
    try:
        if category == 'U21':
            ...
            required_cols = ['Number', 'Name', 'Position', 'Year of Birth', 'Team', 'Conclusion', 'Smart Evaluation', 'KSA Caps']
            for col in required_cols:
                if col not in df.columns:
                    df[col] = '' if col in ['Position', 'Team', 'Conclusion', 'Smart Evaluation'] else 0
            return df
        return pd.DataFrame()
    except Exception as e:
...
def show_fifa_u20_view():
    ...
    with tabs[4]:
        ...
        try:
            ...
            # Try to load reports (match reports)
            try:
            ...
            if selected_conclusion != 'Todas':
                ...
                if not df_reports.empty:
                    if selected_conclusion == 'A - Firmar':
                        ...
                        conclusion_reports = df_reports[df_reports['Conclusion'].str.contains('B\+', case=False, na=False, regex=True)]
                        ...
                        conclusion_reports = df_reports[
                            ...
                            (~df_reports['Conclusion'].str.contains('B\+', case=False, na=False, regex=True))
                        ]
                    else:
                ...
                if not df_individual_reports.empty:
                    if selected_conclusion == 'A - Firmar':
                        ...
                        conclusion_ind_reports = df_individual_reports[df_individual_reports['Conclusion'].str.contains('B\+', case=False, na=False, regex=True)]
                    elif selected_conclusion == 'B - Seguir':
                        conclusion_ind_reports = df_individual_reports[
                            ...
                            (~df_individual_reports['Conclusion'].str.contains('B\+', case=False, na=False, regex=True))
                        ]
                    else:
    ...
    with tabs[5]:
        ...
        # Load match reports and player data
        try:
            df_reports = pd.read_excel('fifa_u20_player_reports.xlsx')
                        ...
                        if st.session_state[match_key]:
                            ...
                            for team_name in teams_in_match:
                                ...
                                for p_idx, report in team_players.iterrows():
                                    ...
                                    if st.session_state[player_key] and full_report_text and str(full_report_text) != 'nan':
                                        report_html = f"""
            ...
            st.info("📊 No reports yet. Create match reports to see them here!" if st.session_state.language == 'en' else "📊 لا توجد تقارير بعد. أنشئ تقارير مباريات لرؤيتها هنا!")
        except Exception as e:
            st.error(f"Error loading reports: {e}")
